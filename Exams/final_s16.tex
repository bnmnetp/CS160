...
\begin{parts}
  \part[3] Identify the base case(s) for writing \texttt{pal} as a recursive function.
  \vspace{1in}
  \part[7] Write a recursive function \texttt{pal(s)} to dermine whether a given string is a palindrome. For example: \texttt{pal("radar")}
...
\part[5] Draw the new heap after you have called the delMin (dequeue) method 2 times.  No need to show the list representation for this part.
\vspace{3in}

\end{parts}

\newpage
\question Given the following directed graph:
\begin{figure}[h!t]
        \begin{center}
        \includegraphics[scale=.65]{dfs}
    \end{center}
\end{figure}

\begin{parts}
\part[10] Perform a depth first search on the graph using node a as the starting vertex. When iterating over neighbors iterate in alphabetical order.  Draw the predecessor links, and add the distance values to the graph.

\part[10] Draw the adjacency matrix representation of the graph.
\vspace{2.0in}

\part[10] Perform a breadth first search on the graph using node a as the starting vertex. When iterating over neighbors iterate in alphabetical order. Draw the predecessor links and add the distance values to the graph.
\begin{figure}[h!t]
        \begin{center}
        \includegraphics[scale=.65]{dfs}
    \end{center}
